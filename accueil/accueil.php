<!DOCTYPE html>
<html lang="fr">
<head>
	<meta charset="UTF-8">
	<link rel="stylesheet" type="text/css" href="../css/bootstrap.min.css">
	<link rel="stylesheet" type="text/css" href="../css/accueil.css">
	<title>Bienvenue</title>
</head>
	<body>
		<hr/>
		   <nav class="navbar navbar-expand-lg navbar-light bg-faded">
		        <div class="collapse navbar-collapse justify-content-center" id="navbarNavAltMarkup">
		        	<div class="navbar-nav">
		              <a class="nav-item nav-link active" href="../accueil/accueil.php">Accueil<span class="sr-only">(current)</span></a>
		              <a class="nav-item nav-link" href="../territoire/territoire.php">Territoire</a>
		              <a class="nav-item nav-link" href="#">Blog</a>
		              <a class="nav-item nav-link disabled" href="#">Contact</a>
		            </div>
		        </div>
		   </nav>
		<hr id="hr2">
		<div class="container rounded" style="border:1px solid #FDFD96;">
			<div class="row">
				<div class="col-md-12">
				<p class="text-center">Notre collectif vous invite à nous faire part de vos besoins et vos envies.<br> Nous sommes tous habitant de la commune et voulons imaginer l’avenir de Cucq Trépied Stella.<br> Agir ensemble sera plus efficace et constructif pour améliorer notre cadre de vie.<br> Partageons ensemble nos idées.</p>
				</div>
			</div> 
		</div>
		<div id="space"></div>
		<div class="container-fluid">
			<div class="row" id="cont">
				<div class="col-md-4"><img src="../img/img1.jpg"></div>
				<div class="col-md-4"><img src="../img/img1.jpg"></div>
				<div class="col-md-4" id="force"><img src="../img/img1.jpg"></div>
			</div>
		</div>
	</body>
</html>